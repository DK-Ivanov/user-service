...
public class UserConsoleController {
     ...
     */
    private final EntityService<UserEntity> entityServiceImpl;

    public UserConsoleController(EntityService<UserEntity> entityServiceImpl) {
    ...
    private void choseCommand(int commandNumber) {
        switch (commandNumber) {
            ...
            case 2 -> update();
            case 3 -> delete();
            case 4 -> showAll(entityServiceImpl.getAll());
            case 5 -> findById();
            case 6 -> findByName();
            case 7 -> findByEmail();
            case 8 -> findByAge();
    ...
    private void create() {
        ...
        entityServiceImpl.save(new UserEntity(userInfo[0].trim(), userInfo[1].trim(), age, LocalDateTime.now()));
    ...
    private void update() {
        ...
        String[] userInfo = readUserInfo();
        int age = parseIntFromConsole(userInfo[2].trim());
        entityServiceImpl.update(new UserEntity(id, userInfo[0].trim(), userInfo[1].trim(), age, entityServiceImpl.findById(id).getCreatedAt()));
    ...
    private int chooseUserById() {
        ...
        List<UserEntity> userEntities = entityServiceImpl.getAll();
    ...
    private void findById() {
        findBy(
                "id",
                id -> List.of(entityServiceImpl.findById(parseIntFromConsole(id.trim())))
        );
    }
    ...
    private void findByName() {
        findBy("имя", entityServiceImpl::findByName);
    }
    ...
    private void findByEmail() {
        findBy("email", email -> List.of(entityServiceImpl.findByEmail(email)));
    }
    ...
    private void findByAge() {
        findBy(
                "возраст",
                age -> entityServiceImpl.findByAge(parseIntFromConsole(age.trim()))
        );
    }
